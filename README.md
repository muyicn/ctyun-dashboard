...
彻底解决了原版必须依赖黑窗口终端命令行交互、单账号繁琐配置、短信验证码绑定困难、自动兑换缺乏界面等所有痛点。
...
1. **多账号独立管理**
   - 支持在 Web 界面添加无限个天翼云账号；
   - 每个账号独立控制：**云电脑保活**、**每日签到**、**AI 对话**、**挂机 1 小时**、**自动兑换/抽奖**。

2. **解决新设备风控短信绑定难题**
   - 原项目在 Docker 中如果遇到风控要求输入短信，容器经常卡死或退出；
   - 本项目当检测到账号未绑定新设备时，卡片高亮提示；点击即可**直接在网页中点击“获取验证码”并在弹窗中提交**，完成后永久信任该设备！

3. **抽奖与自动兑换商品可视化**
   - 自动获取天翼云商城可兑换的商品列表（如“升级 8C16G 电脑配置 (300积分)”、“4C8G升级”、“时长包”、“积分抽奖”等）；
   - 支持灵活的时间策略：
     - **每月指定几号兑换**：输入 `-1` 即表示每月最后一天自动升级配置，实现**全月长期 8C16G** 不降级！
     - **每日兑换**；
     - **每隔 N 天兑换**；
   - 兑换完成后自动触发保活平滑重启，让云电脑立即生效新配置。

4. **实时日志与状态监控**
   - 内置 Server-Sent Events (SSE) 实时日志通道，无需刷新页面，实时查看心跳包、保活握手、挂机进度等；
   - 仪表盘直观展现：当前保活在线数、今日已签到数、已挂机时长。
